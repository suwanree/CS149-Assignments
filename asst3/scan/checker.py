...
def calculate_scores(correct, your_times, fast_times):
    ...
    # 순서대로 1M, 10M, 20M, 40M에 해당하는 K80 Ref Time
    k80_ref_times = [0.766, 8.876, 17.537, 34.754]

    for idx, elem_cnt in enumerate(element_counts):
        # 1. your_times 및 correct 조회 (Dict/List, int/str 키 모두 방어 처리)
        def get_val(data, key, index):
            if isinstance(data, dict):
                if key in data: return data[key]
                if str(key) in data: return data[str(key)]
                try:
                    if int(key) in data: return data[int(key)]
                except: pass
            elif isinstance(data, (list, tuple)) and index < len(data):
                return data[index]
            return None

        stu_time = get_val(your_times, elem_cnt, idx)
        is_correct = get_val(correct, elem_cnt, idx)
        ref_time = get_val(fast_times, elem_cnt, idx)

        # 2. ref_time이 None일 경우 무조건 인덱스(idx) 기준으로 K80 시간 할당
        if ref_time is None:
            ref_time = k80_ref_times[idx] if idx < len(k80_ref_times) else 34.754
