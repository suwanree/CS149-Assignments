...
__global__ void kernelGetCirclesInTile(int* pixelNum) {
...
CudaRenderer::render() {
    ...
    int* pixelNum;
    cudaMalloc(&pixelNum, sizeof(int));
    cudaMemset(pixelNum, 0, sizeof(int));
    kernelGetCirclesInTile<<<gridDim, blockDim>>>(pixelNum);
    cudaDeviceSynchronize();
