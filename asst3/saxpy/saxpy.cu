...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    cudaMemcpy(device_x, xarray, sizeof(float) * N, cudaMemcpyDefault);
    cudaMemcpy(device_y, yarray, sizeof(float) * N, cudaMemcpyDefault);
    ...
    //
    cudaMemcpy(resultarray, device_result, sizeof(float) * N, cudaMemcpyDefault);
    ...
    printf("Kernel time : %.3f ms\n", 1000.f * (endTime_kernel - startTime_kernel));
