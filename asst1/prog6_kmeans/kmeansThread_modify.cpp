...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  int divided = M/numThread;
  int remainder = M%numThread;

  std::thread workers[numThread];
  WorkerArgs args[numThread];
  ...
  for (int i = 0; i<numThread; i++){
    ...
    args[i].mstart = i * divided;
    args[i].mend = (i+1) * divided;

    printf("thread %d assignmented %d ~ %d\n", i, args[i].mstart, args[i].mend);
  }

  if(remainder){
    args[numThread-1].mend = M;
  }
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    for(int i = 1; i<numThread; i++){
      workers[i] = std::thread(computeAssignments, &args[i]);    // O(N)
    }
    ...
    computeCentroids(&args[0]);      // O(N)
    computeCost(&args[0]);           // O(N)
