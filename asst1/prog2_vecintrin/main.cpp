...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  unsigned int remainder = N%VECTOR_WIDTH;
  unsigned int divided = N - remainder;

  __cs149_vec_float result;
  __cs149_vec_float vector;
  __cs149_vec_int exponent;
  __cs149_vec_int iter;
  ...
  __cs149_mask zeros;
  __cs149_mask expMask, clampMask;


  for(int i = 0; i<divided; i+=VECTOR_WIDTH){

    // init vectors
    result = _cs149_vset_float(1.f);
    _cs149_vload_float(vector, values+i, maskAll);
    _cs149_vload_int(exponent, exponents+i, maskAll);
    
    iter = _cs149_vset_int(0);
    expMask = zeros;



    while(_cs149_cntbits(expMask) < VECTOR_WIDTH){
      _cs149_veq_int(expMask, exponent, iter, maskAll);


      expMask = _cs149_mask_not(expMask);

      // exp & iter++
      _cs149_vmult_float(result, result, vector, expMask);
      _cs149_vadd_int(iter, iter, ones, expMask);
      expMask = _cs149_mask_not(expMask);

    }

    _cs149_vgt_float(clampMask, result, clamp, maskAll);
    
    _cs149_vmove_float(result, clamp, clampMask);
    _cs149_vstore_float(output+i, result, maskAll);

  }

  if (remainder){
    clampedExpSerial(values+divided, exponents+divided, output+divided, remainder);
  }
}
...
float arraySumVector(float* values, int N) {
  ...
  // 
  __cs149_vec_float result = _cs149_vset_float(0.0);
  __cs149_vec_float temp;

  __cs149_mask allMask = _cs149_init_ones();

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    _cs149_vload_float(temp, values+i, allMask);

    _cs149_vadd_float(result, result, temp, allMask);
  }

  for (int j = 0; j<int(log2(VECTOR_WIDTH)) - 1; j++){
